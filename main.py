...
class PlayerWidget(Widget):
    def __init__(self, **kwargs):
        ...
        self.idle_speed = 0.5  # ยืนหายใจช้าๆ (สลับภาพทุก 0.5 วินาที)
        self.walk_speed = 0.15  # เดินสับขาเร็วๆ (สลับภาพทุก 0.15 วินาที)

        # สถานะปัจจุบัน
        self.current_anim = self.anim_idle
        self.current_anim_speed = self.idle_speed  # ตั้งค่าเริ่มต้นเป็นความเร็ว Idle
        ...
        with self.canvas:
            ...
            self.rect = Rectangle(
                source=self.current_anim[0], pos=(2500, 2500), size=(64, 64)
            )
    ...
    def set_state(self, is_moving, facing_right=None, current_speed=5.0):
        ...
        # --- คำนวณความเร็ว Animation ตอนเดินให้สอดคล้องกับความเร็วเกม ---
        # กำหนดให้สปีดเดินมาตรฐานของเกมคือ 5.0 (ถ้า speed 5 จะใช้เวลา 0.15 วิ/เฟรม)
        # ถ้า speed เป็น 15 (Dash) ตัวคูณจะเป็น 5/15 = 0.33 (ทำให้เหลือ 0.05 วิ/เฟรม -> เร็วขึ้น 3 เท่า)
        base_speed_ref = 5.0
        safe_speed = max(current_speed, 1.0)  # ป้องกันหารด้วยศูนย์หรือติดลบ
        dynamic_walk_speed = self.walk_speed * (base_speed_ref / safe_speed)

        # เลือกชุดแอนิเมชัน และ ความเร็ว ให้ตรงกับสถานะ
        ...
        if self.current_anim != new_anim:
            ...
            # แม้จะเป็นสถานะเดินเหมือนเดิม ก็ให้อัปเดตความเร็วใหม่เสมอ
            # เพราะผู้เล่นอาจจะกด/ปล่อยปุ่ม Dash ระหว่างที่กำลังเดินอยู่
            self.current_anim_speed = new_speed

    def animate(self, dt):
        self.anim_timer += dt

        # เช็คด้วยความเร็วปัจจุบัน (self.current_anim_speed)
    ...
    def __init__(self, **kwargs):
        ...
        with main_layout.canvas.before:
            ...
            self.bg_rect = Rectangle(
                source="MenuTest.png", pos=(0, 0), size=Window.size
            )
...
class GameScreen(Screen):
    ...
    def update_frame(self, dt):
        ...
        # ส่งค่า current_speed ที่คำนวณแล้วไปให้ระบบ Animation ด้วย
