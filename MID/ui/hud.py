    ...
    def update(self, skills: list):
        ...
        for btn, skill_idx in zip(self.slots, self.skill_indices):
            if skill_idx < len(skills):
                ...
                # เช็คว่าเป็น ActiveSkill (พวกที่มี Stack) หรือ BaseSkill ธรรมดา
                if hasattr(skill, 'current_stacks'):
                    # สกิลประเภทคลิกซ้าย (มี Stack)
                    btn.text = f"{skill.name}\nStacks: {skill.current_stacks}/{skill.max_stacks}"
                    # ถ้า Stack เต็ม หรือมี Stack ให้ใช้ได้ ให้เป็นสีเขียว
                    if skill.current_stacks > 0:
                        btn.background_color = (0.2, 0.6, 0.2, 1)
                    else:
                        btn.background_color = (0.2, 0.2, 0.2, 0.6)
                else:
                    # สกิลประเภทออโต้ (แสดง Cooldown)
...
class HUD(FloatLayout):
    ...
    def _build_debug_buttons(self):
        ...
        debug_specs = [
            ...
            ("CLEAR\nENEMY",  0.58, (0.1, 0.2, 0.3, 0.85), self._clear_enemies),
        ]
    ...
    def update_ui(self, stats):
        ...
        if hasattr(stats, "skills"):
            self.skill_slot_box.update(stats.skills)
    ...
    def _realtime_ui_update(self, _dt):
        ...
        if hasattr(gs, "player_stats") and gs.player_stats:
            ...
            # อัปเดตสกิล Cooldown & Stack
            if hasattr(s, "skills"):
                self.skill_slot_box.update(s.skills)
